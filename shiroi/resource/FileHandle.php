...
//所有接口展示
//获取所有的文件名称,含完整路径
//ShiroiFile::init($path)->getRawFiles();
////获取所有的文件名称,不含路径
//ShiroiFile::init($path)->getFiles();
////获取所有的文件夹名称,含完整路径
//ShiroiFile::init($path)->getRawDirs();
////获取所有的文件夹名称,不含路径
//ShiroiFile::init($path)->getDirs();
////获取目标文件夹$path的树形结构图
//ShiroiFile::init($path)->getTree();
////获取目标文件夹$path的信息
//ShiroiFile::init($path)->getInfo();
////把$path下的所有文件复制到$target目录下,如果有指定类型的情况下,那么只复制指定类型的文件
//ShiroiFile::init($path)->copyFiles($target, 'php');
////把$path下的所有文件夹复制到$target目录下,并且按$path的层级摆放
//ShiroiFile::init($path)->copyDirs($target);
////把$path下的所有文件剪切到$taret目录下,如果有指定类型的情况下,那么只移动指定类型的文件
//ShiroiFile::init($path)->moveFiles($target, 'php');
////把$path下的所有文件及文件夹移动到$target目录下,并且不改变原有的层级结构
//ShiroiFile::init($path)->moveAll($target);
////删除指定文件夹下的所有文件,不含文件夹
//ShiroiFile::init($path)->removeFiles();
////删除指定路径下的所有内容含文件,文件夹
//ShiroiFile::init($path)->removeAll();